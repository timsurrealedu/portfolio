...
function Inspector({ section, project }: { section: SectionId; project: typeof projects[number] }) {
  if (section === "projects") return <div className="inspect-project"><div className="inspect-index">{project.index}</div><p className="inspect-label">SELECTED OBJECT</p><h2>{project.name}</h2><dl><div><dt>TYPE</dt><dd>{project.type}</dd></div><div><dt>STATE</dt><dd>{project.status}</dd></div><div><dt>TAGS</dt><dd>{project.stack.join(" / ")}</dd></div></dl>{project.websiteLinks?.map((link) => <a className="inspect-link" href={link.href} target="_blank" rel="noreferrer" key={link.href}>OPEN SITE →</a>)}{project.href && <a className="inspect-link" href={project.href} target="_blank" rel="noreferrer">VIEW SOURCE →</a>}</div>;
    ...
    <section>
      ...
      <div className="project-layout">
        ...
        <article className="project-detail" aria-live="polite">
          ...
          {project.media && <div className={Array.isArray(project.media) ? "project-media-grid" : undefined}>{(Array.isArray(project.media) ? project.media : [project.media]).map((media) => <figure className="project-media" key={media.src}><img src={media.src} alt={media.alt} /><figcaption>{media.caption}</figcaption></figure>)}</div>}
          <div className="rule" />
          <p>{project.detail}</p>
          <ul className="tag-list">{project.stack.map((item) => <li key={item}>{item}</li>)}</ul>
          {project.websiteLinks?.map((link) => <a className="inline-link" href={link.href} target="_blank" rel="noreferrer" key={link.href}>{link.label} ↗</a>)}
