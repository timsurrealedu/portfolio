    ...
    <section>
      ...
      <div className="experience-group">
        ...
        <div className="organization-list">
          {organizations.map((item) => <article key={item.short}>
            <header><b>{item.short}</b><time>{item.period}</time></header>
            <div><p>{item.relationship}</p><h3>{item.name}</h3><span>{item.summary}</span></div>
      ...
      <div className="evidence-layout">
        <section className="experience-group">
          ...
          {engagements.map((item) => <figure className="event-proof" key={item.role}>
